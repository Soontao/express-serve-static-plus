...
let serveStatic = require('../../src/serve-static.cjs');

let fixtures = path.join(__dirname, '/fixtures');
let relative = path.relative(process.cwd(), fixtures);

let skipRelative = ~relative.indexOf('..') || path.resolve(relative) === relative;
...
describe('serveStatic()', function () {
  ...
  (skipRelative ? describe.skip : describe)('current dir', function () {
    ...
    it('should be served with "."', function (done) {
      let dest = relative.split(path.sep).join('/');
      request(server)
        .get('/' + dest + '/todo.txt')
        .expect(200, '- groceries', done);
    });
  ...
  describe('extensions', function () {
    it('should be not be enabled by default', function (done) {
      let server = createServer(fixtures);
    ...
    it('should be configurable', function (done) {
      let server = createServer(fixtures, { extensions: 'txt' });
    ...
    it('should support disabling extensions', function (done) {
      let server = createServer(fixtures, { extensions: false });
    ...
    it('should support fallbacks', function (done) {
      let server = createServer(fixtures, { extensions: ['htm', 'html', 'txt'] });
  ...
  describe('fallthrough', function () {
    ...
    describe('when true', function () {
      ...
      it('should fall-through when URL too long', function (done) {
        let root = fixtures + Array(10000).join('/foobar');
...
function createServer(dir, opts, fn) {
  ...
  let _serve = serveStatic(dir, opts);
...
function shouldHaveBody(buf) {
  return function (res) {
    let body = !Buffer.isBuffer(res.body)
